...
const InvoiceInqueryPage = () => {
  ...
  const togglePaid = async (row: InvoiceRow) => {
    ...
    try {
      ...
      setResults((prev) => prev.map((r) => (r.id === row.id ? { ...r, status: updated?.status ?? next } : r)));
    } catch (e) {
              ...
              {results.map((row) => {
                ...
                return (
                  <div key={row.id} className="rounded-xl border border-[var(--accent)] overflow-hidden mb-4 bg-black">
                      ...
                      {/* Description panel with items table */}
                      ...
                      <div className="col-span-12 md:col-span-1 flex md:flex-col items-stretch md:items-end gap-2">
                        <button
                          ...
                          className={`${row.status ? 'bg-green-500' : 'bg-[var(--accent)]'} px-4 py-1.5 rounded-md text-black text-xs font-semibold w-full md:w-auto disabled:opacity-60 disabled:cursor-not-allowed`}
                          aria-pressed={row.status ? true : false}
                          title={row.status ? 'Paid (click to mark Unpaid)' : 'Unpaid (click to mark Paid)'}
                        >
                          {row.status ? 'Paid' : 'Unpaid'}
                        </button>
                        <button className="px-4 py-1.5 rounded-md bg-[var(--accent)] text-black text-xs font-semibold w-full md:w-auto">Reprint</button>
                      </div>
                    </div>
                  </div>
                );
